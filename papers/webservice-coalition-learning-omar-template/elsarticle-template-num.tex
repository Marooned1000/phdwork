...
\textbf{Challenge and Problem Statement.} In recent work, communities of web services have been proposed in order to facilitate discovery of web services, improve the Quality of Service (QoS), and help individual services find better market share and opportunities\cite{Zeng:2003:QDW:775152.775211, Paik:2005:TSS:2229263.2230038, Medjahed05adynamic, 10.1109/ARES.2008.7}. However,  two important challenges are to be addressed: 1) choice of best web services during community development from the community perspective and 2) choice of the best community to join from the web service perspective. The advocated solutions \cite{conf/webist/MaamarLBTS07, 10.1109/ARES.2008.7, journals/soca/XuYLZB11, 10.1109/TSC.2012.12, managing-hela-jalel, DBLP:conf/IEEEscc/KhosravifarABT11, DBLP:conf/IEEEscc/LimTMB12} have attempted to address these research problems. However the solutions have not addressed these two challenging issues: 
\begin{enumerate}
...
\end{enumerate}
These approximation methods sometimes negatively influence the outcome because simplifying the constraints may negatively cause important aspects of the problem to be ignored. 
...
\section{Preliminaries and Challenges in WS Communities}\label{s:preliminaries}
...
Sometimes, web services can increase their overall utility by collaborating with other web services as communities. This collaboration helps them with better ways of sharing resources and having higher reputation, better market share and wider visibility. Web services and communities come with different quality metric parameters, and the long-term outcome is dependent on these parameters. 
...
\end{itemize}
	...
	We normalize the range of these features so that each feature contributes proportionally to the final utility outcome value. We adopt the \emph{standardization} method. We subtract the \emph{mean} from each feature then we divide the values of each feature by its \emph{standard deviation}.
...
\begin{itemize}
  \item \emph{External Parameter 1} $(exp_1)$. is an estimate of how close the web service's \emph{execution time} is to other web services. It is the difference between a web service's \emph{execution time} metric and the maximum value of execution time of all the other web services. The smaller value means web service has better execution time compared to other web services.
	\begin{equation}\label{exp_1:f}
		exp_1 = Et_{i} - Et_{max}
	\end{equation}
	\item \emph{External Parameter 2} $(exp_2)$ is a comparison of the web service's rate of performing tasks to other competing web services. It is the difference between a web service's \emph{throughput} metric and the maximum value of throughput of all the other web services. The less value means web service has better throughput compared to other web services.
	\begin{equation}\label{exp_2Lf}
		exp_2 = Th_{max} - Th_{i}
	\end{equation}
\end{itemize}

A community of web services is abstracted as a standard web service that provides services that share the same internal and external features mentioned above for the web services. Inside, they adopt different protocols to manage the community; however, other web services or end users can communicate with communities of web services, using standard web service protocols.
...
When the incoming task rate is less than the throughput of the community, the \emph{weighted fair queuing} algorithm assigns a weighted task rate of $|input Task Rate| \times \frac{Th_{ws}}{\sum_{ws}{Th_{ws}}}$ for each web service ($ws$) within the community.
...
\begin{figure}%[!t]
\centerline{\includegraphics[width=6.0in]{figures/steps.eps}}
...
\textless348, 1934, 2117\textgreater & - & - & - & - & - & - \\ [1ex] % [1ex] adds vertical space
...
K^t(C_i, k) = &\Big\{C_x | C_x >_{i}^t C_y; \forall C_y \in CFVS\ \\
				      &\wedge C_y \notin \{K^t(C_i, 1),K^t(C_i, 2),...,K^t(C_i, k-1)\} \Big\}				
\end{split}
\end{equation}
...
Our goal is to create a decision making profile for each community in the $CFVS$ set. We are creating an environment where they can experience the outcomes of different strategies. The result will be a decision tree of the feasible and utility increasing moves over time. The root of decision tree represents a community in the $CFVS$ set, and the other nodes represents the result of the parent nodes' action of joining other communities and the expected utility of the community. Therefore, each node represents a community with a set of feature values of the parent node's community joining some other community, and an expected utility outcome of the newly formed community.
