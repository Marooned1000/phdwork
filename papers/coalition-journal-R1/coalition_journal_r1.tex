...
their requests to the coordinators of those communities, which are
...
\emph{2) Master Web Services} or the community coordinators, are
...
pool. Community coordinators or master web services are
...
The community master uses a slightly modified \emph{weighted fair
...
While distributing tasks, the community master can verify the
performance, throughput and quality of service of tasks being
performed by web services. The master can recognize if those web
...
throughput, the master will announce the new parameters, which
...
community master will announce the new parameters and community
masters and members can consider those values as benchmark for
...
of $\left\{C_{master_1}, ws_2\right\}$ and $\left\{C_{master_2},
ws_1\right\}$ and a ${ws_3}$ as new web service, ${ws_3}$ joining
${C_{master_1}}$ would hurt at least itself or $ws_2$, however
${ws_3}$ joining ${C_{master_2}}$ would not hurt any participants
...
master's input request rate, it means the web services inside the
...
the players. A community master can subside its community with a
...
community masters so they are interested in the minimum subside
