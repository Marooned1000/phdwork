...
\IEEEPARstart{O}{ver} the past years, online services have become part of many scalable business applications. The increasing reliance on online service providers has significantly influenced the way web services are engineered. Delivering high quality services considering the dynamic and unpredictable nature of the Internet is still a very critical and challenging issue. These services can be utilized on top of intelligent decision making agents in order to maximize gain by finding ways of providing services better satisfying end users. The recent developments are shifting web services from passive and individual components to autonomous and group-based components where interaction, composition, and cooperation are the key challenges [references]. To this end the idea of \emph{community} of web services by gathering services having similar functionality but distinct non-functional properties was developed [references]. 

Web service communities facilitate the discovery of web services and also provide services with better quality. These communities act as abstract web services, communicating with external entities via same standard protocols the normal web services employ. Users send their requests to the coordinators of those communities, which play the role of their representatives or access points. The community coordinator in this \emph{community centralized} architecture is responsible for community management tasks such as verification of credentials of new web services before accepting them to join the community and also firing web services which do not add enough value to the community. Within communities, 

Recent work have emphasized on motivation and benefits of communities of web services[references]. Communities of web services facilitate discovery of web services, improve the quality of service, and also help individual services find better market share and opportunities. A community master or coordinator is also responsible for \emph{community development}. Community development is about finding the right web service for the community, satisfying all web services and community coordinators involved within the community, which is a challenging task. Many ideas have been proposed to tackle this problem. Some of these work have complex on demand algorithms [3way, coopchina, ourwork] and in order to make it feasible they have simplified the task to make the algorithm executable on-demand or they have simplified the constraints, ignoring important aspects of the problem. For example in [coopchina] a simple egalitarian way of distributing gain which completely ignores gain for collaborative work, instead of complex shapely-value which considers gain for all subset of agents working together which basicly ignores the most important criteria of finding optimal group, has been proposed. Other category of related work exists which they do not focus on all parties involved within the community [2player]. 
...
In this paper, we have implemented [OurMethodName] in which we initially train our services in different possible communities and let them experiment and check the outcome of different scenarios. Based on the trained results these services can be utilized with efficient methods of foreseeing how each of their decisions would impact their long term and short term goals, therefore opting for best decision available. 

Web services have different set of features. These features are derived from quality metrics of web services. The values of these features influences the utility of the web services and when collaborating with our web services, these set of features determines the utility and performance of community. Having a set of web services with pre-defined set of features, we let web services evaluate performance of collaboration with a small subset of other web services. Keeping a history of the output estimate of this collaboration, each web service individually will be utilized with knowledge of collaboration with web services having different set of feature values. We keep this history and train web services so they can deduct how they would perform collabration with new web services having new set of feature values. By deploying a linear distant function, the web services will be able to predict the outcome of collaboration with different web services which may not have been collaborated in during training process. 
...
\section{Web Service}\label{s:web_service}
...
*******THIS ONE IS TEMPRORARY FROM PREVIOUS WORKS*******
