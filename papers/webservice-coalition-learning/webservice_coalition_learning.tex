...
We utilize the same communities and web services with a simple rational decision making mechanism, in which communities would choose to join a community if it increases the utility by any amount. Figure \ref{utility_gain_mlisa_and_rational} shows the comparison in end result of utility gain values. In 18 out of 40 tries, rational agents have not been able to improve their utility at all, becuase the communities they have chosen have rejected their request since probably they would not increase the utility of the other communities if they have joined them. The results show a long term strategic decision mechanism is needed to satisfy all the services within communities with the cooperation. Figure \ref{utility_gain_mlisa_and_rational_ratio} shows the results of same experiment as ratio of utility gain.
...
\label{roc5}
\end{figure}
Now we evaluate the system specific results from user and community perspective. By distributing tasks among the communities over these time frames we evaluate the revenue for each community. Figure \ref{stats1} shows the overall revenue gain of communities using our method. Figure \ref{stats2} shows the momentarily gain of revenue for each community on each time slot compared to previous time. These results show how the run with higher learning rate of $r=0.20$ start discovering better communities to join much earlier. The runs with slow rate seems to find some communities to join initially, then slow down until later where they start discovering new communities to join with.
