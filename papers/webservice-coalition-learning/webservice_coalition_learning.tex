...
In the presence of multiple service candidates with identical or similar functionalities, quality of service (QoS) provides nonfunctional characteristics for the optimal candidate selection. Based on the previous investigations [2, 10, 14], we identify the most representative QoS properties of Web services in the following

\begin{itemize}
  \item QoS performance of Web services is highly related to invocation time, since the service status (e.g., workload, number of clients, etc.) and the network environment (e.g., congestion etc.) change over time.
  \item Service users are typically distributed in different geographical locations. The user-observed QoS performance of Web services is greatly influenced by the Internet connections between users and Web services. Different users may observe quite different QoS performance when invoking the same Web service.
\end{itemize}

\subsection{External Features}\label{s:ef}

\subsection{The join challange}\label{s:tjc}

\subsection{Join Consiquences}\label{s:jc}
